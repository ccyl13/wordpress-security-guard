  ...
  return (
    <div className="rounded-xl bg-card border border-border overflow-hidden">
      <div className="px-5 py-4 border-b border-border flex items-center justify-between">
        ...
        <span className={"text-xs font-mono px-2 py-0.5 rounded " + (exposed > 0 ? 'text-red-400 bg-red-400/10' : 'text-emerald-400 bg-emerald-400/10')}>
      ...
      <div className="divide-y divide-border">
        {endpoints.map((ep) => {
          ...
          return (
            <div key={ep.url} className={"px-5 py-3 flex items-start gap-3 hover:bg-secondary/30 transition-colors " + (isAccessible ? 'bg-red-950/10' : '')}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <code className="text-xs text-muted-foreground/40 font-mono">{ep.url.replace(/^https?://[^/]+/, '')}</code>
              </div>
              <div className={"text-xs font-mono px-2 py-0.5 rounded shrink-0 " + (isAccessible ? 'text-red-400 bg-red-400/10' : 'text-emerald-400 bg-emerald-400/10')}>
